...
if __name__ == '__main__':
    ...
    #got averages from get_bell_curve_avg
    ...
    for file in tqdm(os.listdir('dbfiles')):
        ...
        beu, meu, teu, intqeu, moe, sg, stdv, stdu = 0,0,0,0,0,0,0,0

        new_col = []
        wh_info, avgs, std = get_wh_avg_std_data(file)

        wh_fh, wh_sh = get_halves(wh_info)

        #feats: 1-2
        vel_col_fh = get_n_column(wh_fh, 1)
        vel_col_sh = get_n_column(wh_sh, 1)

        if see_trend(vel_col_fh) > 0:
            pvmfh = 1
        if see_trend(vel_col_sh) > 0:
            pvmsh = 1

        #feats: 3-4
        upt_col_fh = get_n_column(wh_fh, 2)
        upt_col_sh = get_n_column(wh_sh, 2)

        if see_trend(upt_col_fh) > 0:
            pufh = 1
        if see_trend(upt_col_sh) > 0:
            push = 1
    
        #feats: 5-8
        col_infor_v = get_n_column(wh_info, 1) #1 is effvel
        bot3v, av, top3v = process_new_sheet(col_infor_v)
        ...
        #feats: 9-12
        col_infor_u = get_n_column(wh_info, 2) #2 is effupt
        bot3u, au, top3u = process_new_sheet(col_infor_u)
        ...
        print('\n', pvmfh, pvmsh, pufh, push, bev, mev, tev, intqv, beu, meu, teu, intqu, moe, sg, stdv, stdu)
        #c = input()
